...
class ProfileManagerCubit extends Cubit<ProfileManagerState> {
  ...
  Future<void> _persistUserProfileData(String userId, String username, String profileImageUrl) async {
    try {
      // Persist profile and publish device public key for E2EE
      await Future.wait([
      ...
      try {
        // Publish per-device X25519 public key to Firestore so other users can
        // fetch it to encrypt messages for this device.
        final keyManager = KeyManager();
        await keyManager.publishPublicKeyToFirestore(_firebaseFirestore, userId);
      } catch (e) {
        debugPrint('Error publishing device public key: $e');
      }
    } catch (e) {
